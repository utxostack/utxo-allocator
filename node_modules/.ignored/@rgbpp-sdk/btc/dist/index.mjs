...
var limitPromiseBatchSize = limitPromiseConcurrency(10);
...
// src/query/source.ts
import { BtcAssetsApiError, ErrorCodes as ServiceErrorCodes } from "@rgbpp-sdk/service";
...
var TxBuilder = class _TxBuilder {
  ...
  async payFee(props) {
    ...
    const originalInputs = cloneDeep3(this.inputs);
    const originalOutputs = cloneDeep3(this.outputs);
    ...
    while (!isFeeExpected) {
      if (isLoopedOnce) {
        ...
        this.inputs = cloneDeep3(originalInputs);
        this.outputs = cloneDeep3(originalOutputs);
      }
  ...
  clone() {
    ...
    tx.inputs = cloneDeep3(this.inputs);
    tx.outputs = cloneDeep3(this.outputs);
    return tx;
  }
  toPsbt() {
...
async function createSendRgbppUtxosBuilder(props) {
  ...
  const rgbppLockArgsList = (await props.ckbCollector.getLiveCells(ckbVirtualTx.inputs.map((input) => input.previousOutput))).map(
    (cell) => isRgbppLockCell(cell.output, isCkbMainnet) ? unpackRgbppLockArgs(cell.output.lock.args) : void 0
  );
