    ...
    constructor(outlen, key, salt, personal) {
        this.update = (input) => {
            if (!(input instanceof Uint8Array)) {
                throw new index_js_1.InputTypeException();
            }
        ...
        this.digest = (out) => {
            ...
            if (!(buf instanceof Uint8Array)) {
                throw new index_js_1.OutTypeException();
            }
...
const blake2b = (outlen, key, salt, personal, noAssert) => {
    if (noAssert !== true) {
        if (outlen < BYTES_MIN) {
            throw new index_js_1.OutLenTooSmallException(outlen, BYTES_MIN);
        }
        if (outlen > BYTES_MAX) {
            throw new index_js_1.OutLenTooLargeException(outlen, BYTES_MAX);
        }
        if (key !== null) {
            if (!(key instanceof Uint8Array)) {
                throw new index_js_1.KeyTypeException();
            }
            if (key.length < KEYBYTES_MIN) {
                throw new index_js_1.KeyLenTooSmallException(key.length, KEYBYTES_MIN);
            }
            if (key.length > KEYBYTES_MAX) {
                throw new index_js_1.KeyLenTooLargeException(key.length, KEYBYTES_MAX);
            }
        }
        if (salt !== null) {
            if (!(salt instanceof Uint8Array)) {
                throw new index_js_1.SaltTypeException();
            }
            if (salt.length !== SALTBYTES) {
                throw new index_js_1.SaltLenException(salt.length, SALTBYTES);
            }
        }
        if (personal !== null) {
            if (!(personal instanceof Uint8Array)) {
                throw new index_js_1.PersonalTypeException();
            }
            if (personal.length !== PERSONALBYTES) {
                throw new index_js_1.PersonalLenException(personal.length, PERSONALBYTES);
            }
        }
    }
