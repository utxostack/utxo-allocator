...
const toHeader = (header) => {
    if (!header)
        return header;
    const { compact_target: compactTarget, transactions_root: transactionsRoot, proposals_hash: proposalsHash, extra_hash: extraHash, parent_hash: parentHash } = header, rest = __rest(header, ["compact_target", "transactions_root", "proposals_hash", "extra_hash", "parent_hash"]);
...
const toLocalNodeInfo = (info) => {
    if (!info)
        return info;
    const { node_id: nodeId, protocols } = info, rest = __rest(info, ["node_id", "protocols"]);
...
const toRemoteNodeInfo = (info) => {
    if (!info)
        return info;
    const { node_id: nodeId, connected_duration: connectedDuration, is_outbound: isOutbound, last_ping_duration: lastPingDuration, sync_state } = info, rest = __rest(info, ["node_id", "connected_duration", "is_outbound", "last_ping_duration", "sync_state"]);
...
const toTxPoolInfo = (info) => {
    if (!info)
        return info;
    const { last_txs_updated_at: lastTxsUpdatedAt, tip_hash: tipHash, tip_number: tipNumber, total_tx_cycles: totalTxCycles, total_tx_size: totalTxSize, min_fee_rate: minFeeRate } = info, rest = __rest(info, ["last_txs_updated_at", "tip_hash", "tip_number", "total_tx_cycles", "total_tx_size", "min_fee_rate"]);
...
const toCapacityByLockHash = (capacityByLockHash) => {
    ...
    const { cells_count: cellsCount, block_number: blockNumber, capacity } = capacityByLockHash, rest = __rest(capacityByLockHash, ["cells_count", "block_number", "capacity"]);
    return Object.assign({ blockNumber,
        capacity,
        cellsCount }, rest);
};
const toBlockEconomicState = (blockEconomicState) => {
    ...
    const { finalized_at: finalizedAt, miner_reward: minerReward, txs_fee: txsFee } = blockEconomicState, rest = __rest(blockEconomicState, ["finalized_at", "miner_reward", "txs_fee"]);
...
const toTransactionProof = (proof) => {
    ...
    const { block_hash: blockHash, witnesses_root: witnessesRoot } = proof, rest = __rest(proof, ["block_hash", "witnesses_root"]);
    return Object.assign({ blockHash,
        witnessesRoot }, rest);
};
const toConsensus = (consensus) => {
    ...
    return {
        ...
        hardforkFeatures: (_b = (_a = consensus.hardfork_features) === null || _a === void 0 ? void 0 : _a.map((_a) => {
            var { epoch_number: epochNumber } = _a, rest = __rest(_a, ["epoch_number"]);
...
const toRawTxPool = (rawTxPool) => {
    ...
    const toTxVerbosity = (_a) => {
        var { ancestors_count: ancestorsCount, ancestors_cycles: ancestorsCycles, ancestors_size: ancestorsSize } = _a, rest = __rest(_a, ["ancestors_count", "ancestors_cycles", "ancestors_size"]);
